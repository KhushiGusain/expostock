...
export default function TopLosersScreen() {
  ...
  const fetchTopLosers = async (showRefreshing = false) => {
    try {
      if (showRefreshing) {
        setRefreshing(true);
      } else {
      ...
      // Format all top losers data for our UI components
      const formattedLosers = response.top_losers.map(formatStockData);
      
      setStockData(formattedLosers);
    } catch (err) {
      const errorMessage = err instanceof Error ? err.message : 'Failed to fetch top losers data';
      ...
      Alert.alert(
        ...
        errorMessage,
        [
          { text: 'Retry', onPress: () => fetchTopLosers() },
  ...
  const renderErrorState = () => (
    <View style={styles.centerContainer}>
      ...
      <Text style={styles.errorText}>Failed to load top losers</Text>
      <TouchableOpacity style={styles.retryButton} onPress={() => fetchTopLosers()}>
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      <View style={styles.header}>
        ...
        <Text style={styles.headerTitle}>Top Losers</Text>
      ...
      >
        ...
        <View style={styles.section}>
          <Text style={styles.sectionSubtitle}>
            {stockData.length} stocks are declining today
          </Text>
          {stockData.length > 0 ? (
            ...
            <View style={styles.emptyState}>
              <MaterialIcons name="trending-down" size={48} color={Colors.light.textTertiary} />
